...
for (const p of pages) fs.copyFileSync(path.join(SRC, `${String(p).padStart(4, '0')}.svg`), path.join(OUT, 'pages', `${String(p).padStart(4, '0')}.svg`));
...
console.log(`staged ${pages.length}/${total} pages into ${path.relative(REPO, OUT)}/ (${(bytes / 1048576).toFixed(1)} MB)`);
