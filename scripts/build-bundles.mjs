...
const RECIPES = [
  ...
  ['cm-type1',   (p) => p.startsWith('fonts/type1/') || p.startsWith('fonts/enc/')],
  ['tex-plain',  (p) => p.startsWith('tex/plain/') || (p.startsWith('tex/generic/') && !p.startsWith('tex/generic/pgf/')) || p === 'web2c/plain.fmt' || p === 'web2c/etex.fmt'],
  ['latex-core', (p) => /^tex\/latex\/(base|l3kernel|l3backend|latexconfig|tex-ini-files)\//.test(p) || p === 'web2c/latex.fmt'],
  ['latex-extra', (p) => p.startsWith('tex/latex/') || p.startsWith('tex/generic/pgf/')],
];
const EAGER = {
