...
script), `data-worker="off"`, `data-observe="off"`, `data-snapshot="on"`. Each
render dispatches a `mp-tikz-wasm:rendered` event, and
...
const mp = await MetaPost.create();          // options: bundles, bundleBaseUrl, numberSystem, tex, snapshot, timeoutMs …
...
and [docs/08](docs/08-javascript-api.md). `sanitizeSvg(svg)` is provided for
